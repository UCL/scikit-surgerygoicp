...
#include <sksgoicpExceptionMacro.h>
#include <iostream>
...
int main(int argc, char **argv)
{
  ...
  try
  {
    ...
    if (deviceCount == 0)
    {
      sksgoicpExceptionThrow() << "No CUDA support compiled in.";
    }
    if (deviceCount == -1)
    {
      sksgoicpExceptionThrow() << "Cannot load CUDA driver, or CUDA is incompatible.";
    }
    ...
    std::cout << "Calculating ... " << sksgoicp::MyFirstAddFunction(1, 2) << std::endl;

    returnStatus = EXIT_SUCCESS;
  }
  catch (sksgoicp::Exception &e)
  {
    std::cerr << "Caught sksgoicp::Exception: " << e.GetDescription() << std::endl;
  }
