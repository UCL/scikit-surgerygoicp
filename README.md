...
would be:
``` cmake
git clone --recursive https://github.com/MattClarkson/scikit-surgerygoicp
mkdir scikit-surgerygoicp-Build
cd scikit-surgerygoicp-Build
ccmake ../scikit-surgerygoicp
make
```
...
```StartVS_Debug.bat``` or ```StartVS_Release.bat``` in the ```SKSURGERYGOICP-build``` folder.
...
* Clone scikit-surgerygoicp, using ```--recursive```.
